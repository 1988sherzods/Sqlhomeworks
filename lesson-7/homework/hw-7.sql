...
Select * from Employees
Select count(*) as employee_count from Employees Group by DepartmentName;

-- 8.
Select * from Products
Select Category, Max(Price) as MaxPrice, Min(Price) As MinPrice from Products Group by Category;
...
Select * from Employees
Select count(*) from Employees Where DepartmentName='HR';
...
Select CustomerID,
Sum(Freight) As TotalFreightAmount,
Min(Freight) As LeastPurchaseAmount
